...
{
	unsigned int crc[2] = {0};
	unsigned char buf_voltage[1024] 	= {0x01, 0x03, 0x0, 0x0, 0x0, 0x02};
	unsigned char buf_temperature[1024] = {0x02, 0x03, 0x0, 0x0, 0x0, 0x02};
	unsigned char buf_ponding[1024] 	= {0x03, 0x03, 0x0, 0x0, 0x0, 0x02};
	crc16(buf_voltage, 6, crc);
	buf_voltage[6] = crc[0];
	buf_voltage[7] = crc[1];
	crc16(buf_temperature, 6, crc);
	buf_temperature[6] = crc[0];
	buf_temperature[7] = crc[1];
	crc16(buf_ponding, 6, crc);
	buf_ponding[6] = crc[0];
	buf_ponding[7] = crc[1];
	switch (event)
	{
		case EVENT_TIMER_TICK:
			...
			{
				case READ_VOLTAGE:
					ServerTCPWrite(hconversation, buf_voltage, 8, 100);
					record_data(buf_voltage, SEND_DATA, READ_VOLTAGE);
					inf_index = READ_VOLTAGE;
					break;
				case READ_TEMPERATURE:
					ServerTCPWrite(hconversation, buf_temperature, 8, 100);
					record_data(buf_temperature, SEND_DATA, READ_TEMPERATURE);
					inf_index = READ_TEMPERATURE;
					break;
				case READ_STATE_PONDING:
					ServerTCPWrite(hconversation, buf_ponding, 8, 100);
